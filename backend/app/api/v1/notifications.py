...
async def get_notification_stats(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        # Calculate rate limiting stats for each channel
        rate_limits = {}
        rate_limit_cutoff = datetime.utcnow() - timedelta(hours=1)

        for channel in all_channels:
            if channel.rate_limit_per_hour:
                # Count notifications sent in the last hour for this channel
                rate_limit_query = select(func.count(NotificationHistory.id)).filter(
                    and_(
                        NotificationHistory.channel_id == channel.id,
                        NotificationHistory.status == "sent",
                        NotificationHistory.created_at >= rate_limit_cutoff
                    )
                )
                rate_limit_result = await db.execute(rate_limit_query)
                sent_last_hour = rate_limit_result.scalar()

                remaining = max(0, channel.rate_limit_per_hour - sent_last_hour)
                is_limited = sent_last_hour >= channel.rate_limit_per_hour

                rate_limits[str(channel.id)] = {
                    "channel_name": channel.name,
                    "limit_per_hour": channel.rate_limit_per_hour,
