...
```json
{
  ...
  "priority": 55,
  "parser_type": "regex",
  "pattern": "^\\[(?P<timestamp>\\d{4}-\\d{2}-\\d{2}\\s+\\d{2}:\\d{2}:\\d{2}\\.\\d{3})\\]\\[(?P<module>[^\\]]+)\\]\\[(?P<log_level>\\w+)\\]\\s+(?P<message>.+?)(?:from\\s+IP:\\s+(?P<client_ip>[\\d.]+))?(?:,\\s+Email:\\s+(?P<email>\\S+))?(?:by\\s+(?P<admin_email>\\S+)\\s+from\\s+(?P<admin_ip>[\\d.]+))?(?:,\\s+Device:\\s+(?P<device>[^,]+))?",
  "field_mappings": {
    "timestamp": "timestamp",
    "module": "module",
    "log_level": "log_level",
...
```json
{
  ...
  "priority": 72,
  "parser_type": "regex",
  "pattern": "^(?P<timestamp>\\w+\\s+\\d+\\s+\\d{2}:\\d{2}:\\d{2})\\s+dnsmasq\\[(?P<pid>\\d+)\\]:\\s+(?:(?P<action>query|reply|cached|forwarded|validation)\\[?(?P<query_type>\\w+)?\\]?\\s+)?(?P<domain>[\\w\\-\\.]+)(?:\\s+(?:is|from)\\s+(?P<resolved_ip>[\\d\\.]+|(?:NODATA|NXDOMAIN|BOGUS)|(?P<client_ip>[\\d\\.]+)))?",
  "field_mappings": {
    "timestamp": "timestamp",
    "pid": "pid",
    "action": "action",
...
VALUES (
  ...
  72,
  'regex',
  '^(?P<timestamp>\w+\s+\d+\s+\d{2}:\d{2}:\d{2})\s+dnsmasq\[(?P<pid>\d+)\]:\s+(?:(?P<action>query|reply|cached|forwarded|validation)\[?(?P<query_type>\w+)?\]?\s+)?(?P<domain>[\w\-\.]+)(?:\s+(?:is|from)\s+(?P<resolved_ip>[\d\.]+|(?:NODATA|NXDOMAIN|BOGUS)|(?P<client_ip>[\d\.]+)))?',
