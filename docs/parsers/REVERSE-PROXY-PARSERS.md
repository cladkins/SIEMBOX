...
```json
{
  ...
  "priority": 60,
  "parser_type": "regex",
  "pattern": "^(?<client_ip>[\\d.]+)\\s+-\\s+(?<username>\\S+)\\s+\\[(?<timestamp>[^\\]]+)\\]\\s+\"(?<http_method>\\S+)\\s+(?<request_uri>\\S+)\\s+(?<http_version>[^\"]+)\"\\s+(?<http_status>\\d+)\\s+(?<bytes_sent>\\d+)\\s+\"(?<referrer>[^\"]*)\"\\s+\"(?<user_agent>[^\"]+)\"",
  "field_mappings": {
    "client_ip": "client_ip",
    "username": "username",
    "timestamp": "timestamp",
...
```json
{
  ...
  "priority": 61,
  "parser_type": "regex",
  "pattern": "^(?<timestamp>\\d{4}/\\d{2}/\\d{2}\\s+\\d{2}:\\d{2}:\\d{2})\\s+\\[(?<severity>\\w+)\\]\\s+(?<pid>\\d+)#(?<tid>\\d+):\\s+\\*(?<connection_id>\\d+)\\s+(?<error_message>.+?),\\s+client:\\s+(?<client_ip>[\\d.]+),\\s+server:\\s+(?<server_name>\\S+),\\s+request:\\s+\"(?<request>[^\"]+)\"(?:,\\s+upstream:\\s+\"(?<upstream>[^\"]+)\")?",
  "field_mappings": {
    "timestamp": "timestamp",
    "severity": "severity",
    "pid": "pid",
...
VALUES (
  ...
  60,
  'regex',
  '^(?<client_ip>[\d.]+)\s+-\s+(?<username>\S+)\s+\[(?<timestamp>[^\]]+)\]\s+"(?<http_method>\S+)\s+(?<request_uri>\S+)\s+(?<http_version>[^"]+)"\s+(?<http_status>\d+)\s+(?<bytes_sent>\d+)\s+"(?<referrer>[^"]*)"\s+"(?<user_agent>[^"]+)"',
...
VALUES (
  ...
  61,
  'regex',
  '^(?<timestamp>\d{4}/\d{2}/\d{2}\s+\d{2}:\d{2}:\d{2})\s+\[(?<severity>\w+)\]\s+(?<pid>\d+)#(?<tid>\d+):\s+\*(?<connection_id>\d+)\s+(?<error_message>.+?),\s+client:\s+(?<client_ip>[\d.]+),\s+server:\s+(?<server_name>\S+),\s+request:\s+"(?<request>[^"]+)"(?:,\s+upstream:\s+"(?<upstream>[^"]+)")?',
...
```json
{
  ...
  "priority": 75,
  "parser_type": "regex",
  "pattern": "^(?<client_ip>[\\d.]+)\\s+-\\s+(?<username>\\S+)\\s+\\[(?<timestamp>[^\\]]+)\\]\\s+\"(?<http_method>\\S+)\\s+(?<request_uri>\\S+)\\s+(?<http_version>[^\"]+)\"\\s+(?<http_status>\\d+)\\s+(?<bytes_sent>\\d+)\\s+\"(?<referrer>[^\"]*)\"\\s+\"(?<user_agent>[^\"]+)\"(?:\\s+\"(?<upstream>[^\"]+)\")?",
  "field_mappings": {
    "client_ip": "client_ip",
    "username": "username",
    "timestamp": "timestamp",
...
```json
{
  ...
  "priority": 76,
  "parser_type": "regex",
  "pattern": "^(?<timestamp>\\d{4}/\\d{2}/\\d{2}\\s+\\d{2}:\\d{2}:\\d{2})\\s+\\[(?<severity>\\w+)\\]\\s+(?<pid>\\d+)#(?<tid>\\d+):\\s+\\*(?<connection_id>\\d+)\\s+(?<error_message>.+?),\\s+client:\\s+(?<client_ip>[\\d.]+),\\s+server:\\s+(?<server_name>\\S+),\\s+request:\\s+\"(?<request>[^\"]+)\"(?:,\\s+upstream:\\s+\"(?<upstream>[^\"]+)\")?(?:,\\s+host:\\s+\"(?<host>[^\"]+)\")?",
  "field_mappings": {
    "timestamp": "timestamp",
    "severity": "severity",
    "pid": "pid",
...
VALUES (
  ...
  75,
  'regex',
  '^(?<client_ip>[\d.]+)\s+-\s+(?<username>\S+)\s+\[(?<timestamp>[^\]]+)\]\s+"(?<http_method>\S+)\s+(?<request_uri>\S+)\s+(?<http_version>[^"]+)"\s+(?<http_status>\d+)\s+(?<bytes_sent>\d+)\s+"(?<referrer>[^"]*)"\s+"(?<user_agent>[^"]+)"(?:\s+"(?<upstream>[^"]+)")?',
...
VALUES (
  ...
  76,
  'regex',
  '^(?<timestamp>\d{4}/\d{2}/\d{2}\s+\d{2}:\d{2}:\d{2})\s+\[(?<severity>\w+)\]\s+(?<pid>\d+)#(?<tid>\d+):\s+\*(?<connection_id>\d+)\s+(?<error_message>.+?),\s+client:\s+(?<client_ip>[\d.]+),\s+server:\s+(?<server_name>\S+),\s+request:\s+"(?<request>[^"]+)"(?:,\s+upstream:\s+"(?<upstream>[^"]+)")?(?:,\s+host:\s+"(?<host>[^"]+)")?',
