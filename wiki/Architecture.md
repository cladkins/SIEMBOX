...
| **Backend** | Node.js + TypeScript + Express | REST API, syslog listener, parser/rules engines, scanners, the **SIEMBOX Endpoint server** + YARA delivery, the **AI Security Analyst** tool loop, and jobs (port **8421**, syslog **514**). |
| **Database** | PostgreSQL (JSONB) | Raw + parsed logs, parsers, rules, alerts, assets, vulnerabilities, threat feeds/indicators, endpoint agents + YARA bundles, analyst chat threads, settings. |
| **Log shipper** | Alpine container | *Optional* forwarder installed on other hosts (see [Log Shippers](Log-Shippers)). |
| **SIEMBOX Endpoint agent** | Separate lightweight agent | *Optional* agent enrolled from the UI; reports inventory/detections/vulns and runs server-delivered YARA (see [SIEMBOX Endpoint](SIEMBOX-Endpoint)). |
...
- **`edr_agents`, `edr_enrollment_tokens`, `edr_yara_bundle`** — endpoint agents, one-time enrollment tokens, and versioned YARA bundles.
