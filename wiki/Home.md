...
A lightweight, self-hosted **Security Information and Event Management (SIEM)** system built with Node.js/TypeScript, Vue 3, and PostgreSQL. **v3** adds an **AI Security Analyst** and **SIEMBOX Endpoint agents** on top of the v2 *Parser Platform*, where parsers and detection rules are *portable data*: shareable, installable from a community catalog, and generatable by AI.
...
| **SIEMBOX Endpoint** | Enroll endpoint agents for host inventory, detections, and vulnerability scans, with server-delivered **YARA** rule packs | [SIEMBOX Endpoint](SIEMBOX-Endpoint) |
